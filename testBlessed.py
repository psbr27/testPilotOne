...
test_data = [
    TestResult(
        "G11", "AutoCreateSubs", "test_auto_create_subs_1", "GET", "PASS", 0.21
    ),
    TestResult("G11", "SLFGroups", "test_add_slf_group", "PUT", "PASS", 0.23),
    TestResult("G11", "SLFGroups", "test_add_slf_group", "GET", "PASS", 0.21),
    TestResult(
        "G11", "SLFGroups", "test_modify_slf_group", "PUT", "PASS", 0.22
    ),
    TestResult(
        "G11", "SLFGroups", "test_modify_slf_group", "GET", "PASS", 0.22
    ),
    TestResult(
        "G11", "SLFGroups", "test_delete_slf_group", "DELETE", "FAIL", 0.21
    ),
    TestResult(
        "G11", "SLFGroups", "test_delete_slf_group", "GET", "FAIL", 0.25
    ),
    TestResult(
        "G11", "DefaultGroupId", "test_default_group_id_1", "PUT", "PASS", 0.21
    ),
    TestResult(
        "G11", "DefaultGroupId", "test_default_group_id_1", "GET", "PASS", 0.24
    ),
    TestResult(
        "G11", "DefaultGroupId", "test_default_group_id_2", "GET", "PASS", 0.22
    ),
    TestResult(
        "G11", "DefaultGroupId", "test_default_group_id_3", "GET", "FAIL", 0.23
    ),
    TestResult(
        "G11", "DefaultGroupId", "test_default_group_id_4", "PUT", "PASS", 0.24
    ),
...
def demo_blessed_fixed():
    ...
    try:
        ...
        print(
            "Notice how it automatically shows only the last N results that fit in your terminal"
        )
        time.sleep(2)
...
def demo_blessed_with_scrollback():
    ...
    try:
        ...
        def display_tests(results_list, offset=0):
            ...
            print("=" * min(100, term.width - 1))
            print(
                f"{term.dim}Use ↑/↓ arrows to scroll, 'q' to quit{term.normal}"
            )
            ...
            if offset > 0:
                scroll_info = (
                    f" | {term.yellow}Scrolled up {offset} lines{term.normal}"
                )
            elif total > available_height:
                scroll_info = (
                    f" | {term.yellow}Showing latest results{term.normal}"
                )
        ...
        print(
            "This will show how to handle large result sets with user scrolling"
        )
        time.sleep(2)
...
def demo_blessed_live_with_autoscroll():
    ...
    try:
        ...
        def display_tests(results_list):
            ...
            available_height = (
                term.height - 6
            )  # Reserve space for headers/footers
            ...
            for i, result in enumerate(visible_results):
                ...
                style = (
                    term.reverse
                    if i == len(visible_results) - 1 and len(results_list) > 1
                    else ""
                )
