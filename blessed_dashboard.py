...
class BlessedDashboard:
    ...
    def _calculate_layout(self):
        ...
        if current_height != self.last_terminal_height:
            ...
            self.max_visible_rows = max(1, current_height - self.header_height - self.footer_height)
            self.force_full_redraw = True
    ...
    def _get_visible_results(self):
        ...
        if self.auto_scroll:
            ...
            start_idx = max(0, len(self.results) - max_rows - self.scroll_offset)
    ...
    def _update_display_with_autoscroll(self):
        ...
        need_full_redraw = (
            self.force_full_redraw or
            len(visible_results) != self.last_displayed_count or
            len(self.results) > max_rows and len(self.results) % max_rows == 1
        )
    ...
    def _format_progress_line(self):
        ...
        if len(self.results) > max_rows:
            ...
            progress += f" | Showing latest {max_rows} (+ {hidden_count} hidden)"
...
class BlessedProgressOnly:
    ...
    def add_result(self, test_result):
        ...
        if len(progress.replace(self.term.green, "").replace(self.term.red, "").replace(self.term.yellow, "").replace(self.term.normal, "")) > self.term.width:
            ...
            if self.dry_run > 0:
                progress += f" | {self.term.yellow}◦{self.dry_run}{self.term.normal}"
