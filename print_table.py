...
def display_test_results_with_fixed_and_wrapped_alignment(
    results_data, delay_seconds=1, max_entries=100
):
    """
    ...
    for header in headers_list:
        width = fixed_col_widths.get(
            header, len(header)
        )  # Use fixed width, fallback to header length
    ...
    for i, row_data in enumerate(results_data):
        ...
        wrapped_index = textwrap.wrap(
            index_val, width=fixed_col_widths["Index"]
        )
        wrapped_sheet_name = textwrap.wrap(
            sheet_name_val, width=fixed_col_widths["Sheet Name"]
        )
        ...
        wrapped_result = textwrap.wrap(
            result_val, width=fixed_col_widths["Result"]
        )
        wrapped_fail_reason = textwrap.wrap(
            fail_reason_val, width=fixed_col_widths["Fail Reason"]
        )
        ...
        for line_num in range(max_lines):
            ...
            line_parts.append(
                f" {wrapped_index[line_num] if line_num < len(wrapped_index) else '':<{fixed_col_widths['Index']}}"
            )
            line_parts.append(
                f" {wrapped_sheet_name[line_num] if line_num < len(wrapped_sheet_name) else '':<{fixed_col_widths['Sheet Name']}}"
            )
            line_parts.append(
            ...
            if line_num < len(wrapped_result):
                line_parts.append(
                    colorize_and_pad(
                        wrapped_result[line_num], fixed_col_widths["Result"]
                    )
                )
            else:
                line_parts.append(" " * (fixed_col_widths["Result"] + 1))
            line_parts.append(
                f" {wrapped_fail_reason[line_num] if line_num < len(wrapped_fail_reason) else '':<{fixed_col_widths['Fail Reason']}}"
            )
        ...
        print(
            separator_line
        )  # Print separator after each logical row (which might be multi-line)
...
class TestDisplayRow:
    ...
    duration: float = 0.0
    fail_reason: str = ""
    _original_test_name: str = (
        ""  # Store the original test name before truncation
    )
...
class PrintTableDashboard:
    ...
    def __init__(
        self, mode="full", auto_scroll=True, max_entries=100, delay_seconds=0.1
    ):
        self.mode = mode
    ...
    def _display_single_result(self, result_dict, show_header=True):
        ...
        if show_header:
            display_test_results_with_fixed_and_wrapped_alignment(
                [result_dict], delay_seconds=0, max_entries=1
            )
        else:
            # Get data with defaults (ensure string types)
            ...
            if isinstance(duration_raw, (int, float)) or (
                ...
                and duration_raw.replace(".", "", 1).isdigit()
            ):
                try:
                    duration_val = f"{float(duration_raw):.2f}"
                except ValueError:
            ...
            for line_num in range(max_lines):
                ...
                if line_num < len(wrapped_result):
                    ...
                    if result_text == "PASS":
                        line_parts.append(
                            f" \033[92m{result_text:{fixed_col_widths['Result']}}\033[0m"
                        )
                    elif result_text == "FAIL":
                        line_parts.append(
                            f" \033[91m{result_text:{fixed_col_widths['Result']}}\033[0m"
                        )
                    else:
                        line_parts.append(
                            f" {result_text:<{fixed_col_widths['Result']}}"
                        )
                else:
    ...
    def _display_progress(self, row):
        ...
        status_char = (
            "✓"
            if row.status == "PASS"
            else "✗" if row.status == "FAIL" else "◦"
        )
        status_color = (
            "\033[92m"
            if row.status == "PASS"
            else "\033[91m" if row.status == "FAIL" else "\033[93m"
        )
        reset_color = "\033[0m"
    ...
    def _display_simple(self, row):
        ...
        if row.status == "FAIL" and row.fail_reason:
            print(
                f"  {status_color}Fail reason: {row.fail_reason}{reset_color}"
            )
        ...
        for sheet_name, stats in sheet_results.items():
            ...
            if stats["failed"] > 0:
                ...
                for result in stats["failed_tests"]:
                    ...
                    full_test_name = getattr(
                        result, "_original_test_name", result.test_name
                    )
