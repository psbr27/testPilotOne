...
def generate_test_results(count=25):
    ...
    test_names = [
        "User_Authentication", "Profile_Update", "Product_Creation", 
        "Order_Processing", "Payment_Gateway", "Data_Validation",
        "Cache_Refresh", "Health_Check", "Load_Balancer", "API_Rate_Limit",
        "Session_Management", "Database_Connection", "File_Upload",
        "Email_Service", "Notification_System", "Backup_Process",
        "Security_Scan", "Performance_Test", "Integration_Test", "Unit_Test"
    ]
...
def demo_mode(mode, description):
    ...
    try:
        # Create dashboard
        if mode == "full":
            dashboard = create_blessed_dashboard(mode="full", max_visible_rows=15)
        elif mode == "progress":
...
def main():
    ...
    while True:
        try:
            choice = input(f"\nSelect demo (1-{len(modes)}) or 'q' to quit: ").strip().lower()
