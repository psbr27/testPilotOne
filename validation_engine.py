...
def check_diff(context: 'ValidationContext') -> Optional['ValidationResult']:
    ...
    if diff:
        return ValidationResult(False, f"Response payload does not match expected payload. Difference: {diff}")
        ...
        if context.method and context.method.upper() == "PUT":
            try:
                if context.expected_status is None or context.actual_status is None:
                    ...
                    return ValidationResult(False, "Expected status or actual status is None")
                ...
                if (expected_status_int in (200, 201)) and (actual_status_int in (200, 201)):
                    ...
                    logger.info(f"Status mismatch: {actual_status_int} != {expected_status_int}")
                    return ValidationResult(False, f"Status mismatch: {actual_status_int} != {expected_status_int}")
            except Exception:
                logger.info(f"Unable to compare status: {context.actual_status} vs {context.expected_status}")
                return ValidationResult(False, f"Unable to compare status: {context.actual_status} vs {context.expected_status}")
        elif context.actual_status != context.expected_status:
            logger.info(f"Status mismatch: {context.actual_status} != {context.expected_status}")
            return ValidationResult(False, f"Status mismatch: {context.actual_status} != {context.expected_status}")
        if context.response_body != context.response_payload:
            logger.info("Response body does not match response payload, running check_diff")
            ...
            if diff_result is not None:
                ...
                return ValidationResult(False, "Unknown error during payload comparison")
...
class PutStatusAndPatternValidator(ValidationStrategy):
    def validate(self, context: ValidationContext) -> ValidationResult:
        ...
        if context.method and context.method.upper() == "PUT":
            ...
            return ValidationResult(False, f"Status mismatch: {context.actual_status} != {context.expected_status}")
        found = False
        if context.pattern_match and context.pattern_match in str(context.response_body):
            logger.info(f"Pattern '{context.pattern_match}' found in response body")
            found = True
        elif context.pattern_match and context.response_headers and context.pattern_match in str(context.response_headers):
            logger.info(f"Pattern '{context.pattern_match}' found in response headers")
            found = True
        if not found:
            logger.info(f"Pattern '{context.pattern_match}' not found in response body or headers")
            return ValidationResult(False, f"Pattern '{context.pattern_match}' not found in response body or headers")
...
class PutStatusPayloadPatternValidator(ValidationStrategy):
    def validate(self, context: ValidationContext) -> ValidationResult:
        ...
        result = match_patterns_in_headers_and_body(context.pattern_match, context.response_headers, context.response_body, logger)
...
class ValidationDispatcher:
    ...
    def dispatch(self, context: ValidationContext) -> ValidationResult:
        self.logger.info(f"Dispatching validation for method={context.method}, is_kubectl={context.is_kubectl}, "
        ...
        if context.method.upper() == 'PUT':
            if context.expected_status and not context.response_payload and not context.pattern_match:
                self.logger.info("Selected strategy: put_status_only")
                result = VALIDATION_STRATEGIES['put_status_only'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
            if context.expected_status and context.response_payload and not context.pattern_match:
                self.logger.info("Selected strategy: put_status_and_payload")
                result = VALIDATION_STRATEGIES['put_status_and_payload'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
            if context.expected_status and context.pattern_match and not context.response_payload:
                self.logger.info("Selected strategy: put_status_and_pattern")
                result = VALIDATION_STRATEGIES['put_status_and_pattern'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
            if context.expected_status and context.response_payload and context.pattern_match:
                self.logger.info("Selected strategy: put_status_payload_pattern")
                result = VALIDATION_STRATEGIES['put_status_payload_pattern'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
        # GET rules
        if context.method.upper() == 'GET':
            if context.saved_payload is not None:
                self.logger.info("Selected strategy: get_compare_with_put (workflow-aware GET)")
            ...
            if context.expected_status and context.response_payload and not context.pattern_match:
                ...
                result = VALIDATION_STRATEGIES['get_status_and_payload'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
            if context.expected_status and context.pattern_match and not context.response_payload:
                self.logger.info("Selected strategy: get_status_and_pattern")
                result = VALIDATION_STRATEGIES['get_status_and_pattern'].validate(context)
                self.logger.info(f"Validation outcome: passed={result.passed}, reason={result.fail_reason}")
                return result
        # DELETE rules
        ...
        if context.is_kubectl and context.pattern_match:
            self.logger.info("Selected strategy: kubectl_pattern (kubectl log validation)")
...
class KubectlPatternValidator(ValidationStrategy):
    def validate(self, context: ValidationContext) -> ValidationResult:
        ...
        if missing:
            ...
            return ValidationResult(False, f"Patterns not found in kubectl output: {missing}")
...
def match_patterns_in_headers_and_body(pattern: Optional[str], headers: Optional[Dict[str, Any]], body: Optional[Any], logger) -> ValidationResult:
    """
    ...
    headers_dict = {k.lower(): str(v) for k, v in headers.items()} if isinstance(headers, dict) else {}
    ...
    for key, val in patterns:
        ...
        if key:  # key:value style
            ...
            if header_val and val in header_val:
                logger.info(f"Pattern key='{key}' with value='{val}' matched in headers")
                matched = True
        ...
        if not matched:
            logger.warning(f"Pattern key='{key}' or value='{val}' NOT matched in headers or body")
            return ValidationResult(False, f"Pattern key='{key}' or value='{val}' NOT matched in headers or body")
