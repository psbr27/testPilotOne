...
class PatternMatchParser:
    ...
    def extract_pattern_matches(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for sheet_name, df in self.workbook_data.items():
            ...
            for idx, row in df.iterrows():
                ...
                pattern_value = (
                    row.iloc[col_index] if col_index < len(row) else None
                )

                # Only include non-empty, non-NaN values
                if (
                    pd.notna(pattern_value)
                    and str(pattern_value).strip() != ""
                    and str(pattern_value).lower() != "pattern_match"
                ):
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        total_patterns = sum(
            len(patterns) for patterns in self.pattern_match_data.values()
        )
    ...
    def export_enhanced_data(
        self, enhanced_data: Dict[str, Any], output_file: str = None
    ) -> None:
        """
        ...
        if output_file is None:
            output_file = (
                self.excel_file_path.stem + "_enhanced_pattern_matches.json"
            )
...
def main():
    ...
    from pattern_to_dict_converter import (
