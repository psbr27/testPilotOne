...
def main():
    ...
    print(
        f"  • Successful conversions: {conv_stats['successful_conversions']}"
    )
    print(
        f"  • Success rate: {conv_stats['successful_conversions']/conv_stats['total_processed']*100:.1f}%"
    )
...
def demonstrate_data_access(enhanced_data):
    ...
    for sheet_name, patterns in enhanced_patterns.items():
        for pattern_entry in patterns:
            converted = pattern_entry["converted_pattern"]
            if (
                "enabled" in converted.get("data", {})
                or converted["pattern_type"] == "boolean"
            ):
