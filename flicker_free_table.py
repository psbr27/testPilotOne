...
class ProgressOnlyDisplay:
    ...
    def print_final_summary(self, all_results=None):
        ...
        for result in passed_tests[:10]:  # Show first 10
            print(f"  {getattr(result, 'test_name', 'Unknown')} on {getattr(result, 'host', 'Unknown')}")
...
class BatchedTable:
    ...
    def _print_single_result(self, test_result):
        ...
        test_name = getattr(test_result, "test_name", "")[:self.column_widths['test_name']-1]
        method = getattr(test_result, "method", "")[:self.column_widths['method']-1]
        ...
        if hasattr(test_result, "result") and getattr(test_result, "result", "") == "DRY-RUN":
            result = f"{ANSI_YELLOW}DRY-RUN{ANSI_RESET}" if self.supports_ansi else "DRY-RUN"
        elif getattr(test_result, "passed", False):
    ...
    def print_final_summary(self, all_results=None):
        ...
        print("\n" + "=" * SEPARATOR_WIDTH)
        print(f"FINAL SUMMARY: {passed} PASSED | {failed} FAILED | {len(self.all_results)} TOTAL")
        print("=" * SEPARATOR_WIDTH)
...
class StaticPlusLiveTable:
    ...
    def _update_display(self):
        ...
        print("=" * 80)
        print(f"Total: {len(self.all_results)} | Passed: {passed} | Failed: {failed} | Rate: {rate:.1f}/sec")
        print()
    ...
    def print_final_summary(self, all_results=None):
        ...
        print("FINAL TEST RESULTS")
        print("=" * SEPARATOR_WIDTH)
        print(f"SUMMARY: {passed} PASSED | {failed} FAILED | {len(self.all_results)} TOTAL")
        print("=" * SEPARATOR_WIDTH)
