...
def check_flexible_log_pattern_v3(output, pattern_match):
    ...
    for line in output.strip().split("\n"):
        ...
        try:
            ...
            logger.debug(
                f"Checking log entry with expected {level} and {logger_name}: {log_entry}"
            )
            # Match level and logger
            if (
                log_entry.get("level") == level
                and log_entry.get("loggerName") == logger_name
            ):
    ...
    logger.debug(
        f"Found {matches_found} entries with matching level/logger but no matching key phrases"
    )
    return False
