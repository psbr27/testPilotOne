...
def validate_host_config(host_config: Dict[str, Any]) -> None:
    ...
    for field in required_fields:
        if field not in host_config or not host_config[field]:
            raise ValueError(f"Host configuration missing required field: {field}")
...
def mask_sensitive_data(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sensitive_fields = ['password', 'key_file', 'private_key', 'secret', 'token']
...
def create_example_env_file(hosts_config: Dict[str, Any], output_path: str = '.env.example') -> None:
    """
    ...
    content = ["# Auto-generated TestPilot environment variables example",
