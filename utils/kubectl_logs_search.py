...
def search_similar_patterns(output_string, base_pattern=r'User-Agent[:\s]*"[^"]*"'):
    """
...
def search_in_custom_output(output_string, pattern_match, case_sensitive=True):
    ...
    try:
        ...
        # Display results
        if result['found']:
            print(f"✓ Pattern '{pattern_match}' found {result['count']} time(s)!")
            for i, pos in enumerate(result['positions']):
                print(f"  Match {i+1}: '{pos['match']}' at position {pos['start']}-{pos['end']}")
        else:
            print(f"✗ Pattern '{pattern_match}' not found in the output string.")
