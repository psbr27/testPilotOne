...
class CurlOutputParser:
    ...
    def _is_progress_line(self, line):
        ...
        return (line.startswith('%') or 
                'Total' in line and 'Received' in line and 'Speed' in line or
                re.match(r'^\s*\d+\s+\d+\s+\d+\s+\d+', line))
    ...
    def _parse_request_line(self, line):
        ...
        if re.match(r'^(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)', line):
            ...
            self.request_info[key.strip().lower().replace('-', '_')] = value.strip()
    ...
    def _parse_response_body(self, line):
        ...
        if not line.startswith('{') and '[' not in line and 'bytes data' not in line:
            self.response_body.append(line)
    
    def _is_timing_line(self, line):
        """Check if line contains timing information"""
        return (re.match(r'^\s*\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+', line) or
                'speed' in line.lower() and 'time' in line.lower())
...
def print_universal_analysis(parsed_data):
    ...
    if headers:
        # Categorize headers
        standard_headers = ['date', 'server', 'content-type', 'content-length', 'cache-control']
        security_headers = ['x-frame-options', 'x-xss-protection', 'strict-transport-security']
        custom_headers = [k for k in headers.keys() if k.lower() not in standard_headers + security_headers]
