...
def compare_json_objects(
    json1: Union[str, dict],
    json2: Union[str, dict],
    comparison_type: str = "structure_and_values",
    ...
    Args:
        ...
        ignore_array_order: If True, arrays are compared ignoring element order (default: True)
    ...
    if comparison_type == "structure_only":
        return compare_structure_only(obj1, obj2, ignore_array_order)
    elif comparison_type == "values_only":
        return compare_values_only(obj1, obj2, ignore_array_order)
    elif comparison_type == "structure_and_values":
        return compare_structure_and_values(obj1, obj2, ignore_array_order)
    elif comparison_type == "deep":
        return deep_compare(obj1, obj2, ignore_array_order)
    else:
        raise ValueError("Invalid comparison_type")


def compare_structure_only(
    obj1: Any, obj2: Any, ignore_array_order: bool = True
) -> Dict[str, Any]:
    ...
    def get_structure(obj, path=""):
        if isinstance(obj, dict):
            return {
                f"{path}.{k}" if path else k: get_structure(
...
def compare_values_only(
    ...
    if values1 == values2:
        ...
        percentage = (
            (common_chars / max_length * 100) if max_length > 0 else 100
        )
...
def compare_structure_and_values(
    obj1: Any, obj2: Any, ignore_array_order: bool = True
) -> Dict[str, Any]:
    """Compare both structure and values."""

    def normalize_array_for_comparison(arr):
        """Normalize array for order-independent comparison"""
        if not ignore_array_order:
            return arr

        # Create a normalized version for comparison
        # For simple values, sort them. For complex objects, use a different approach
        try:
            # Try to sort if all elements are simple (strings, numbers, booleans)
            if all(
                ...
                # For complex objects, sort by string representation
    ...
    def flatten_json(obj, parent_key="", sep="."):
        ...
        if isinstance(obj, dict):
            ...
            if ignore_array_order:
                # For order-independent comparison, normalize the array first
                normalized_list = normalize_array_for_comparison(obj)
                for i, v in enumerate(normalized_list):
                    # Use a consistent key that doesn't depend on original position
                    new_key = f"{parent_key}[item_{i}]"
                    items.extend(flatten_json(v, new_key, sep=sep).items())
            else:
                # Original behavior - order-sensitive
        ...
        if key in flat1 and key in flat2:
            if flat1[key] == flat2[key]:
                ...
                mismatched_keys.append(
                    {"key": key, "value1": flat1[key], "value2": flat2[key]}
                )
        else:
            missing_keys.append(
                {
    ...
    percentage = (
        (matching_pairs / total_pairs * 100) if total_pairs > 0 else 100
    )
...
def deep_compare(
    ...
    def deep_compare_recursive(o1, o2, path=""):
        results = {"matches": 0, "total": 0, "differences": []}
        ...
        if isinstance(o1, dict):
            ...
            for key in all_keys:
                ...
                if key not in o1:
                    ...
                    sub_result = deep_compare_recursive(
                        o1[key], o2[key], current_path
                    )
            ...
            if ignore_array_order:
                    ...
                    results["differences"].append(
                        {
                            ...
                            "value1": f"length {len(o1)}",
                            "value2": f"length {len(o2)}",
                        }
                    )
                else:
                    # Try to match each element in o1 with an element in o2
                    used_indices = set()
                    for i, item1 in enumerate(o1):
                        found_match = False
                        for j, item2 in enumerate(o2):
                            if j in used_indices:
                                continue
                            # Compare items
                            temp_result = deep_compare_recursive(
                                item1, item2, f"{path}[{i}→{j}]"
                            )
                            if (
                                temp_result["total"] == temp_result["matches"]
                            ):  # Perfect match
                                results["matches"] += temp_result["matches"]
                                results["total"] += temp_result["total"]
                                used_indices.add(j)
                                found_match = True
                                break
                ...
                for i in range(max_len):
                    ...
                    if i >= len(o1):
                        ...
                        sub_result = deep_compare_recursive(
                            o1[i], o2[i], current_path
                        )
                        results["matches"] += sub_result["matches"]
                        results["total"] += sub_result["total"]
                        results["differences"].extend(
                            sub_result["differences"]
                        )
    ...
    percentage = (
        (result["matches"] / result["total"] * 100)
        if result["total"] > 0
        else 100
    )
